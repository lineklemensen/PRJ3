...
std::pair<double, double> DR::calc_route(std::pair<double, double> next_pos)
{
    ...
    std::pair<double, double> next_pos_vector = {
        next_pos.first - current_pos.first, next_pos.second - current_pos.second
    };
    ...
    double angle = (pow(length_next_vector, 2) + pow(length_last_vector, 2) - pow(length_last_to_next, 2)) / (
                       2 * length_last_vector * length_next_vector);
    ...
    } else if(current_pos.second > next_pos.second && last_pos.second > current_pos.second && last_pos.first <
              current_pos.first) {
        cos_angle = cos_angle * -1;
    } else if(current_pos.second < next_pos.second && last_pos.first > current_pos.first) {
        cos_angle = cos_angle * -1;
    } else if(current_pos.second > next_pos.second && last_pos.second > next_pos.second && last_pos.first < current_pos.
              first) {
        cos_angle = cos_angle * -1;
    } else if(current_pos.first > next_pos.first && last_pos.second > current_pos.second && last_pos.first > next_pos.
              first) {
        cos_angle = cos_angle * -1;
    }
